...
//    protected float radius;
    ...
    public Pig(World world, float x, float y,float hx ,float hy) {
        ...
        pigShape.setAsBox(hx,hy); // Standard size
        this.hx=hx;
        this.hy=hy;
        FixtureDef pigFixture = new FixtureDef();
        pigFixture.shape = pigShape;
        pigFixture.density = 7f;
        pigFixture.friction = 0.2f;
        pigFixture.restitution = 0.1f; // Bounciness
        velocityX=0;
        velocityY=0;
    ...
    public void render(SpriteBatch batch) {
        if (texture != null) {
            if(status){
                ...
                batch.draw(texture, position.x - hx, position.y - hy, 2*hx, 2*hy);
            }
